...
RUN apt-get -qq -y update && \
    ...
    curl -L "https://www.mongodb.org/static/pgp/server-${MONGODB_VERSION_REPO}.asc" | gpg --dearmor -o /usr/share/keyrings/mongo.gpg && \
    echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongo.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/${MONGODB_VERSION_REPO} multiverse" | tee /etc/apt/sources.list.d/mongodb-org-${MONGODB_VERSION_REPO}.list && \
    # Elasticsearch
    ...
    apt -y remove gpg gpg-agent && apt-get clean -y && apt-get autoclean -y && apt-get autoremove -y && \
